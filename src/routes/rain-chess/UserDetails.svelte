<script lang="ts">
  ...
  $: if ($signer) {
    let tokenData = JSON.parse(
      localStorage.getItem("oauth2authcodepkce-state")
    );
    getAccountData(tokenData);
  }
  const getAccountData = async (tokenData) => {
    winNFTCtrct = new ethers.Contract(
      ContractsConfigs.winTokenNFT,
      ContractsConfigs.NFTABI,
      $signer
    );
    gmNFTCtrct = new ethers.Contract(
      ContractsConfigs.gmTokenNFT,
      ContractsConfigs.NFTABI,
      $signer
    );
    expNFTCtrct = new ethers.Contract(
      ContractsConfigs.expTokenNFT,
      ContractsConfigs.NFTABI,
      $signer
    );
    imprNFTCtrct = new ethers.Contract(
      ContractsConfigs.impTokenNFT,
      ContractsConfigs.NFTABI,
      $signer
    );
    ...
    let context = await axios.post(
      "https://gildlab-ipfs.in.ngrok.io/lichess/api/v2/getNftContext",
      { lichessToken: tokenData?.accessToken?.value }
    );
  ...
  const mintNFT = async (badge) => {
    ...
    if (badge == "win") {
      let tx = await winNFTCtrct.flow(
        ContractsConfigs.winTokenNFTFlowId,
        getNFTContext.username,
        [getNFTContext.nftContext]
      );
      let receipt = await tx.wait();
    } else if (badge == "gm") {
      let tx = await gmNFTCtrct.flow(
        ContractsConfigs.gmTokenNFTFlowId,
        getNFTContext.username,
        [getNFTContext.nftContext]
      );
      let receipt = await tx.wait();
    } else if (badge == "xp") {
      let tx = await expNFTCtrct.flow(
        ContractsConfigs.expTokenNFTFlowId,
        getNFTContext.username,
        [getNFTContext.nftContext]
      );
      let receipt = await tx.wait();
    } else if (badge == "improve") {
      let tx = await imprNFTCtrct.flow(
        ContractsConfigs.impTokenNFTFlowId,
        getNFTContext.username,
        [getNFTContext.nftContext]
      );
      let receipt = await tx.wait();
...
{#if $signer}
  <Section>
    ...
    <div class="p-3 pl-5">
      {#if !accDetails?.profile}
        Username : <span class="uppercase font-semibold"
          ><a
            target="_blank"
            class="text-blue-400 underline"
            href={accDetails?.url}
            >{accDetails ? accDetails?.username : "John Doe"}</a
          ></span
        >
      {:else}
        Name : <span class="uppercase font-semibold"
          ><a
            target="_blank"
            class="text-blue-400 underline"
            href={accDetails?.url}
            >{`${
              accDetails
                ? accDetails?.profile?.firstName +
                  " " +
                  accDetails?.profile?.lastName
                : "John Doe"
            }`}</a
          ></span
        >
            ...
            <div class="grid grid-cols-3 gap-x-4 gap-y-8 py-4">
              <div class="flex items-center gap-x-4">
                ...
                <div class="flex flex-col">
                  ...
                  <div class="text-2xl">
                    {`${accDetails ? accDetails?.count.all : 0}`}
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-x-4">
                ...
                <div class="flex flex-col">
                  ...
                  <div class="text-2xl">
                    {`${accDetails ? accDetails?.count.win : 0}`}
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-x-4">
                ...
                <div class="flex flex-col">
                  ...
                  <div class="text-2xl">
                    {`${accDetails ? accDetails?.count.draw : 0}`}
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-x-4">
                ...
                <div class="flex flex-col">
                  ...
                  <div class="text-2xl">
                    {`${accDetails ? accDetails?.count.loss : 0}`}
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-x-4">
                ...
                <div class="flex flex-col">
                  ...
                  <div class="text-2xl">
                    {`${accDetails ? accDetails?.count.playing : 0}`}
                  </div>
                </div>
              </div>
            </div>
  ...
  <Section>
    ...
    <SectionBody>
      <div
        class="flex flex-row gap-x-2 items-center  bg-violet-200 rounded-lg self-start p-3 max-w-prose ml-4"
      >
        <IconLibrary width={30} icon="tip" />
        <div class="max-w-prose">
          Mint Badges, if you have <span class="font-bold"
            >GM, WIN, XP and IMPROVE</span
          > token balance !
        </div>
      </div>
      <div class="grid grid-cols-4 gap-x-4 gap-y-4 px-4">
        <Section>
          <SectionBody>
            ...
            {:else if winMetaData}
              <img
                src={winMetaData?.data?.animation_url}
                width="144"
                height="144"
                alt="all"
                class="flex self-center bg-black"
              />
            {/if}
          </SectionBody>
        </Section>
        <Section>
          <SectionBody>
            ...
            {:else if gmMetaData}
              <img
                src={gmMetaData?.data?.animation_url}
                width="144"
                height="144"
                alt="all"
                class="flex self-center bg-black"
        ...
        <Section>
          <SectionBody>
            ...
            {:else if expMetaData}
              <img
                src={expMetaData?.data?.animation_url}
                width="144"
                height="144"
                alt="all"
                class="flex self-center bg-black"
        ...
        <Section>
          <SectionBody>
            ...
            {:else if imprMetaData}
              <img
                src={imprMetaData?.data?.animation_url}
                width="144"
                height="144"
                alt="all"
                class="flex self-center bg-black"
